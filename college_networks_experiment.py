#Code to run simulations of the Facebook100 networks
import numpy as np
import pandas as pd
...
class edge_weight_DW:
    ...
    #('adjacency' refers to the networks we create from adjacency matrixes, i.e., our real-world networks)
    folder_names = {'complete': 'Complete', 'erdos-renyi': 'Erdos-Renyi', 'degree-regular': 'Degree-Regular', 'adjacency': 'Adjacency'}
    
    random_graph_types = ["complete", "erdos-renyi"]

    def __init__(self, graph_type, n, college_name = "", p=False):
        """
        blah blah 
        """
        ...
        if self.graph_type == "degree-regular":
                self.k = k
                if not os.path.exists(f"{self.foldername}/matfiles"):
                    os.makedirs(f"{self.foldername}/matfiles")
                
                if not os.path.exists(f"{self.foldername}/txtfiles"):
                    os.makedirs(f"{self.foldername}/txtfiles")

        if self.graph_type == "adjacency":
            self.foldername = f"college-networks"
            self.subfolder_name = college_name
            self.adjacency_matrix_name = f"{self.subfolder_name}_matrix.csv"
            
            if not os.path.exists(f"{self.foldername}/{self.subfolder_name}"):
                os.makedirs(f"{self.foldername}/{self.subfolder_name}")
            
            if not os.path.exists(f"{self.foldername}/{self.subfolder_name}/matfiles"):
                os.makedirs(f"{self.foldername}/{self.subfolder_name}/matfiles")
                
            if not os.path.exists(f"{self.foldername}/{self.subfolder_name}/txtfiles"):
                os.makedirs(f"{self.foldername}/{self.subfolder_name}/txtfiles")
    ...
    def generate_seed_files(self):
        ...
        self.graph_seed_file = f"{self.foldername}/{self.subfolder_name}/graph_seeds.csv"
        self.opinion_seed_file = f"{self.foldername}/{self.subfolder_name}/opinion_seeds.csv"
        ...
        if self.graph_type == "complete":
            ...
            #We generate one opinion seed for k-regular cycle-like graph
            if not os.path.exists(self.opinion_seed_file):
                df = pd.DataFrame(columns = ['opinion_seed'])
                random.seed(a=None) #reset random by seeding it with the current time
                weight_seed = str(random.randrange(sys.maxsize))
        ...
        elif self.graph_type == "adjacency":
            #We generate one opinion seed per real-world network
            if not os.path.exists(self.opinion_seed_file):
                df = pd.DataFrame(columns = ['opinion_seed'])
                random.seed(a=None) #reset random by seeding it with the current time
                weight_seed = str(random.randrange(sys.maxsize))
        ...
        if self.graph_type =='degree-regular':
            #self.sim_seed_file = f"{self.foldername}/sim_seeds/k-{k}.csv"
            self.sim_seed_file = f"{self.foldername}/sim_seeds.csv"
        if self.graph_type == 'adjacency': 
            self.sim_seed_file = f"{self.foldername}/{self.subfolder_name}/sim_seeds.csv"
    ...
    def run_DW(self, params):
        ...
        For k-regular cycle-like graphs, we also specify "k", the common degree of the graph.
        '''
        ...
        if not os.path.exists(f"{self.foldername}/{self.subfolder_name}/matfiles/{sub_folder}"):
            os.makedirs(f"{self.foldername}/{self.subfolder_name}/matfiles/{sub_folder}")
        if not os.path.exists(f"{self.foldername}/{self.subfolder_name}/txtfiles"):
            os.makedirs(f"{self.foldername}/{self.subfolder_name}/txtfiles")
        ...
        #LEILA: replace below with != erdos-renyi
        ...
        txtfile = f"{self.foldername}/{self.subfolder_name}/txtfiles/{savename}.txt"
        ...
        def make_ring_lattice(n,k):
            #if want multiple values of n, should add list(range(n))? 
            nodes = range(n)
            G = igraph.Graph()
        ...
        #Functions to make graph from adjacency matrix: 
        def make_adjacency_matrix():
            A = pd.read_csv(self.adjacency_matrix_name, header=None)
            A = A.values
            G = igraph.Graph.Adjacency(A.astype(bool).tolist(), mode = "undirected")
        ...
        folder = f"/delta{delta}-gamma{gamma}/"
        matfile = f"{self.foldername}/{self.subfolder_name}/matfiles{folder}{savename}-op{opinion_set}.mat"
...
## EXPERIMENT PARAMETERS - CHANGE HERE
    ...
    n = 2970 # caltech: 762; reed: 962; Smith: 2970
    college_name = f"Smith"
    ...
    zs = [0.1, 0.5, 0.9] #Initial Edge-Weight
                    ...
                    for z in zs: 
                        for opinion_set in opinion_sets:
                            if graph_type == "adjacency":
                                matfile = (f"college-networks/{college_name}/matfiles/"
f"delta{delta}-gamma{gamma}/--delta{delta}-gamma{gamma}--c{c}--z{z}-mu{mu}-op{opinion_set}.mat")
                            else: 
                                matfile = (f"{graph_type}{n}/matfiles/delta{delta}-gamma{gamma}/"
                             f"--delta{delta}-gamma{gamma}--c{c}--z{z}-mu{mu}-op{opinion_set}.mat")
                                #edit for different graph types (such as by adding 'p' or 'k'
                            try:
    ...
    for gamma in gammas:
        for c in cs:
            for mu in mus:
                for opinion_set in opinion_sets:
                    if graph_type == "adjacency":
                        matfile = (f"college-networks/{college_name}/matfiles/delta{1}-gamma{0}/"
    ...
    experiment = edge_weight_DW('adjacency', n = n, college_name = college_name, p=False)
    experiment.generate_seed_files()

    l = multiprocessing.Lock()

    with multiprocessing.Pool(processes=65, initializer=init, initargs=(l,)) as pool:
